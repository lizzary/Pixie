    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-950">
      ...
      <div className="flex items-center gap-4 px-6 py-4 border-b border-gray-800 shrink-0">
        ...
        <h2 className="text-lg font-semibold text-gray-100">
          搜索: <span className="text-purple-400">{query}</span>
        </h2>
        {results && <span className="text-sm text-gray-500">{results.total} 个结果</span>}
      </div>

      {/* Content */}
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          <div className="flex items-center justify-center h-64 text-gray-500 text-sm">搜索中...</div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-600">
            ...
            <p className="text-sm">没有找到匹配的插画</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {results.items.map((ill) => (
              <div key={ill.id} className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
                <div className="aspect-square bg-gray-800 flex items-center justify-center overflow-hidden">
                  {imgErrors[ill.id] ? (
                    <span className="text-gray-600 text-xs">加载失败</span>
                  ) : (
                    <img
