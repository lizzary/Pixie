...
- **Python 3.10+** 含 `pip`
- **Node.js 18+** 含 `npm`
- （可选）NVIDIA GPU 支持 CUDA，用于加速 AI 打标
...
cd frontend
npm install
npm start                    # 启动于 http://localhost:3000
```

- **后端** → `http://localhost:8000`（Swagger 文档位于 `/docs`）
- **前端** → `http://localhost:3000`

### 生产模式（单服务器）

```bash
cd frontend && npm run build   # 生成 build/ 目录
cd ../backend
python -m uvicorn main:app --host 127.0.0.1 --port 8000
# 访问 http://127.0.0.1:8000 —— 前端和 API 均由同一源提供服务
```
...
| 层级 | 技术 |
|-------|-----------|
| **后端** | Python, FastAPI, SQLite (含 FTS5) |
| **AI 打标** | PyTorch, timm, HuggingFace Hub |
| **前端** | React 19, Tailwind CSS 3, Framer Motion, Lucide React |
| **路由** | React Router v7 |
| **图像处理** | Pillow (PIL) |
